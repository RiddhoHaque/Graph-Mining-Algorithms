...
int main(){
	...
	for(auto tuple: tuples){
		...
		for(auto graph: database){
			...
			parentExpectedSupport.push_back(1.0);

		}
        for(int i=parentExpectedSupport.size()-2; i>=0; i--){
            parentExpectedSupport[i]+=parentExpectedSupport[i+1];
        }
		DFSCode code;
		code.tuples.push_back(tuple);
		if(!isMin(code)) continue;
		gSpan(code, embeddings, parentExpectedSupport);
		for(auto graph: database){
